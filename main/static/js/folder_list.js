  ...
  beforeMount: function () {
    ...
    this.data.map((item,ind) =>{ 
      if(!item['type']||item['type'] ==null ){
        item['type'] = langChoose =='ja' ? "指定なし" : "Unselected"
      }
      if(item['type'] =="unselected"){
        item['type'] = "Unselected"
      }
    })
    this.listCategories = [...new Set(this.data.map(item => item['type']))].filter(a => a != null);
    this.listStatus = [/*"Loading", "Executed" ,"Error" ,*/"Confirmed" ]
  },
  mounted() {
    var self = this;
    ...
    let arrLink = [{'link': '#','name': langChoose =='ja' ?'資料管理' :'Management Document'},{'link': '#','name':langChoose =='ja' ?'ファイルリスト':'File List'}]
  ...
  methods: {
    ...
    statusFilter() {
      ...
      let data = this.data.filter(function (history) {
        ...
        return (parseInt(history.rate_confirmed.substring(0, history.rate_confirmed.lastIndexOf('/'))) > 0);
      });
    ...
    deleteSelected(){
      this.name_delete = this.selectedList.length + ' folders'
      this.typedelete = 1;
      $('#alert').modal('toggle');
    },
    confirmDelete() {
    ...
    exportDataByKey(key) {
      ...
      if (lisTmp.length > 0) {
        ...
        var fd = new FormData();
        fd.append("data", lisTmp);
        xhr.open("POST", "/export_multiple_package", true);
        xhr.send(fd);
      }
    },
    exportData() {
